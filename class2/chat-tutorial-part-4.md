...
So now we need to trigger `scrollToBottom()` when new messages come in. Meteor provides the `onRendered()` hook to execute code whenever a template has been placed into the DOM<sup>[2](#what-is-DOM)</sup> - which will happen whenever a new message is added. Let's use that to call `scrollToBottom()`.  Replace `/*chat window scrolling*/` with:
...
Making the button actually scroll the window is very simple. Just add this event directly under the `scroll event`:  Replace the `/* more messages event */` with
...
<a name="what-is-callback">1</a>: A callback function is a function which is passed as an argument to another function and is invoked after some kind of event.  Once its parent function completes, the function passed as an argument is then called. -- [8bitjunkie on stackoverflow](http://stackoverflow.com/questions/824234/what-is-a-callback-function)

<a name="what-is-dom">2</a>: [HTML DOM (Document Object Model)](http://www.w3schools.com/js/js_htmldom.asp) is a W3 standard for Javascript to access and update HTML elements, attributes, CSS style and events in a page
